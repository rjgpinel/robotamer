...
def main(args):
    ...
    try:
        ...
        for seed in tqdm(range(args.init_seed, args.init_seed + args.poses)):
            ...
            targets_pos = []
            for i in range(num_cubes):
                target_pos = compute_target_pos(sim_obs, target_key=f"cube{i}_pos")
                stats[f"target{i}_pos"].append(target_pos)
            ...
            with open(str(real_output_dir / f"{seed:07d}.pkl"), "wb") as f:
                pkl.dump((real_processed_obs, {f"target{i}_pos": targets_pos[i] for i in range(num_cubes)}), f)

            with open(str(sim_output_dir / f"{seed:07d}.pkl"), "wb") as f:
                pkl.dump((sim_processed_obs, {f"target{i}_pos": targets_pos[i] for i in range(num_cubes)}), f)
